...
const InvoicesManager = () => {
  ...
  const create = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin_invoices'] }); toast.success('Invoice created'); setOpen(false); setForm(empty); },
  ...
  const updateStatus = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin_invoices'] }); toast.success('Updated'); },
  });
  ...
  return (
    <div>
        ...
        <Dialog open={open} onOpenChange={o => { setOpen(o); if (!o) setForm(empty); }}>
          ...
          <DialogContent>
            ...
            <div className="space-y-4">
              ...
              <div><Label>User ID</Label><Input value={form.user_id} onChange={e => setForm({ ...form, user_id: e.target.value })} placeholder="UUID of user" /></div>
              <div className="flex gap-4">
                ...
                <div className="w-24"><Label>Currency</Label><Input value={form.currency} onChange={e => setForm({ ...form, currency: e.target.value })} /></div>
              </div>
      ...
      <Table>
        <TableHeader><TableRow><TableHead>Invoice #</TableHead><TableHead>Shipment</TableHead><TableHead>Amount</TableHead><TableHead>Status</TableHead><TableHead>Date</TableHead><TableHead>Actions</TableHead></TableRow></TableHeader>
        <TableBody>
