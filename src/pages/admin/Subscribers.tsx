  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-foreground">Subscribers ({subscribers?.length || 0})</h1>
        ...
        <TableBody>
          {subscribers?.map((s) => (
            <TableRow key={s.id}>
              ...
              <TableCell><span className={s.active ? 'text-primary' : 'text-destructive'}>{s.active ? 'Yes' : 'No'}</span></TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
